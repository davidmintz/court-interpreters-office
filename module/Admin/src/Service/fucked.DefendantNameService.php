...
class DefendantNameService
{
    ...
    public function update(Entity\Defendant $entity, array $data, array $options = []) : array
    {
        ...
        // the first question is whether there is a duplicate
            ...
            switch ($match) {
                case null:
                    ...
                    if ($update_type == self::UPDATE_GLOBAL) {
                        ...
                        $id = $result['insert_id'] = $db->lastInsertId();
                        $result = array_merge($result, $this->doRelatedTableUpdates((int)$id, $entity->getId(),
                             $contexts_submitted));
                        // $data['id'] = $id;
                        // $result['entity'] = $data; $result['entity']['id'] = $id;
