...
class EventControllerTest extends AbstractControllerTest
{
    ...
    public function testUpdateInCourtEvent(Entity\Event $entity)
    {
        ...
        $id =  $em->createQuery('SELECT e.id FROM InterpretersOffice\Entity\Event e ORDER BY e.id DESC')
    ...
    {
        $db = Bootstrap::getEntityManager()->getConnection();
        $when = (new \DateTime('-10 minutes'))->format("Y-m-d H:i:s");
        $id = Bootstrap::getEntityManager()->createQuery(
            'SELECT MIN(e.id) FROM InterpretersOffice\Entity\Event e'
        )->getSingleScalarResult();
        $db->executeQuery("UPDATE events SET modified = '$when' WHERE id = $id");

        $this->login('david', 'boink');
        $this->reset(true);
        $this->dispatch('/admin/schedule/edit/'.$id);
        // sanity
        $this->assertResponseStatusCode(200);
